...
declare function client_state(): any;

export let speech_enabled: boolean = false;
export let voice: string | null = null;

/**************************************************************************
 is Speech Synthesis API enabled?
**************************************************************************/
export function is_speech_supported(): boolean {
...
**************************************************************************/
export function speak_unfiltered(text: string): void {
  if (!is_speech_supported() || !speech_enabled) return;
...
/**************************************************************************
 Say the given text using the Speech Synthesis API
**************************************************************************/
export function speech_synth(text: string, filter_enabled: boolean): void {
  if (!is_speech_supported() || !speech_enabled) return;

  const text_filtered: string | null = filter_enabled ? filter_speech_messages(text) : text;
  ...
  if (voice == null) {
    ...
    voices.forEach(function(voice_check: SpeechSynthesisVoice) {
  ...
  if (voice != null) {
    msg.voice = speechSynthesis.getVoices().filter(function(voice_check: SpeechSynthesisVoice) { return voice_check.name === voice; })[0];
  }
...
**************************************************************************/
export function filter_speech_messages(input: string | null): string | null {
