/**********************************************************************
    Freeciv-web - the web version of Freeciv. https://www.freeciv.org/
    Copyright (C) 2009-2017  The Freeciv-web project

    This program is free software: you can redistribute it and/or modify
    it under the terms of the GNU Affero General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU Affero General Public License for more details.

    You should have received a copy of the GNU Affero General Public License
    along with this program.  If not, see <http://www.gnu.org/licenses/>.

***********************************************************************/

declare const $: any;
declare function swal(message: string): void;
declare const username: string;
declare const nations: any;
declare const civserverport: number;
declare const client: any;
declare const game_info: any;
declare function client_is_observer(): boolean;
declare function is_longturn(): boolean;
declare function get_score_text(pplayer: any): number;

export let submitted_to_hof: boolean = false;

/**************************************************************************
 Submit game to Hall of Fame
**************************************************************************/
export function submit_game_to_hall_of_fame(): void {
  if (client_is_observer() || client.conn.playing == null || submitted_to_hof || is_longturn() || $.getUrlVar('action') == "multi") {
    return;
  }
  submitted_to_hof = true;

  const pplayer: any = client.conn.playing;

  if (game_info['turn'] < 30 || get_score_text(pplayer) == 0) {
    return;
  }

  $.ajax({
   type: 'POST',
   url: "/hall_of_fame_post?username=" + username + "&nation=" + nations[pplayer['nation']]['adjective'] + "&score=" + get_score_text(pplayer)
         + "&turn=" + game_info['turn'] + "&port=" + civserverport  ,
   success: function(data: any, textStatus: any, request: any){
       $("#hof_msg").html("Game submitted to Hall of Fame! See it <a href='/hall_of_fame' target='_new'>here</a>.");
   }

   }).fail(function() {
    swal("Unable to sumit game to Hall of Fame. Please try again later!");
    $.unblockUI();
  });
}
