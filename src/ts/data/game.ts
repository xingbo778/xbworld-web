...
export function update_game_status_panel(): void {
  ...
  if (($(window as any) as any).width() - sum_width() > 800) {
    ...
    if ($("#game_status_panel_bottom").length) {
      ...
      $("#game_status_panel_bottom").css("width", ($(window as any) as any).width());
      $("#game_status_panel_bottom").html(status_html);
    }
  }

  let page_title = "XBWorld - " + username
...
export function sum_width(): number {
  let sum = 0;
  $("#tabs_menu").children().each(function (this: HTMLElement) {
