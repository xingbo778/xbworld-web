...
{
  if (tiles == null || civclient_state < C_S_RUNNING) return;
  if (!sprites_init) init_cache_sprites();
  if (active_city != null) return;
  ...
  var tw = tileset_tile_width;
  var th = tileset_tile_height;

  for (var tid_str in dirty_tiles) {
    var tid = parseInt(tid_str, 10);
    var ptile = tiles[tid];
    if (ptile == null) continue;

    var r = map_to_gui_pos(ptile['x'], ptile['y']);
    var cx = r['gui_dx'] - mapview['gui_x0'];
    var cy = r['gui_dy'] - mapview['gui_y0'];
