/***********************************************************************
    XBWorld - Packet handler dispatch table and client_handle_packet.

    These were previously generated by Closure Compiler and embedded in
    webclient.min.js. Now that we load source files directly, they need
    to be defined explicitly.

    This file must be loaded AFTER packhand.js (which defines all the
    handle_* functions) and BEFORE any code that calls
    client_handle_packet (e.g., clinet.js's websocket onmessage).
***********************************************************************/
...
/**
 * Main packet dispatch function. Called by the WebSocket onmessage handler.
 * Iterates over the array of packets and dispatches each to the appropriate
 * handler based on its packet ID (pid).
 *
 * @param {Array} packets - Array of packet objects, each with a 'pid' field.
 */
function client_handle_packet(packets) {
  ...
  try {
    for (var i = 0; i < packets.length; i++) {
      if (packets[i] != null) {
        packet_hand_table[packets[i].pid](packets[i]);
      }
    }
    if (packets.length > 0) {
